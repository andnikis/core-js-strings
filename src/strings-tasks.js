...
function getFirstChar(value) {
  // correct for strings, value must be passed
...
function removeFirstOccurrences(str, value) {
  const replaceFrom = str.indexOf(value);

  return replaceFrom >= 0
    ? str.slice(0, replaceFrom) + str.slice(replaceFrom + value.length)
    : str;
}
...
function removeLastOccurrences(str, value) {
  const replaceFrom = str.lastIndexOf(value);
...
function reverseString(str) {
  // standard solution using an array
...
function countVowels(str) {
  ...
  return str.split('').reduce((acc, cur) => {
    // no-param-reassign eslint for acc
    const count = vowels.includes(cur) ? acc + 1 : acc;
    return count;
  }, 0);
}
...
function isPalindrome(str) {
  // delete punctuation marks and spaces
  const clearStr = str.replaceAll(/[ ?!,.]+/g, '').toLowerCase();
...
function reverseWords(str) {
  ...
  // save each reversed word in the array
  words.forEach((v) => reversedWords.push(v.split('').reverse().join('')));
...
function invertCase(str) {
  ...
  for (let i = 0, len = str.length; i < len; i += 1) {
    ...
    const invertedChar =
      str[i] !== upperChar ? upperChar : str[i].toLowerCase();
    chars[i] = invertedChar;
  }
...
function unbracketTag(str) {
  // there should be only one char "<" and ">" in str
...
function encodeToRot13(str) {
  // result length is the same
  const encoded = new Array(str.length);
  const codeA = 'a'.charCodeAt(0);
  const codeZ = 'z'.charCodeAt(0);

  for (let i = 0, len = str.length; i < len; i += 1) {
    const c = str[i].toLowerCase().charCodeAt(0);
    // only alphabet
    if (c >= codeA && c <= codeZ)
      encoded[i] = String.fromCharCode(
        str[i].charCodeAt(0) + (str[i].toLowerCase() <= 'm' ? 13 : -13)
      );
    // ignore punctuation marks
    else encoded[i] = str[i];
  }
